...
public class Solution {
    public static boolean checkTelNumber(String telNumber) {
        ...
        if(telNumber.matches("(\\+|\\+-|-)(\\d{2}(\\(\\d{3}\\)|\\d{3})-?(\\d{3}\\(?\\d{3}\\)?-?\\d|\\d{3}-?\\d{2}-?\\d{2}|\\d{2}-?\\d{2}-?\\d{3}))")) return  true;
        if(telNumber.matches("\\+(\\(\\d{3}\\)-?(\\d-?\\d{7}-?\\d))")) return  true;
        if(telNumber.matches("\\(?\\d{3}\\)?-?\\d{3}-?\\d{2}-?\\d{2}|\\d{6}\\(?\\d{3}\\)?-?\\d|\\d-\\d{8}-\\d|\\d-\\d{5}-\\d{4}")) return  true;
